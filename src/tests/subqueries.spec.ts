...
describe('Subqueries', () => {
    ...
    it('[bugfix] can select select = subqueries with min()', () => {
        ...
        setupBooks();
        expect(many(`SELECT name FROM books WHERE created_at = (SELECT MIN(created_at) FROM books);`)).to.deep.equal([
            { name: 'one' },
            { name: 'four' },
        ]);
    });
    ...
    it('can = subquery when one resulting row', () => {
        setupBooks();
        expect(
            many(`SELECT name FROM books WHERE created_at = (SELECT created_at FROM books WHERE created_at > 2)`),
    ...
    it('cannot = subquery when multiple resulting row', () => {
        setupBooks();
        assert.throws(
            () => none(`SELECT name FROM books WHERE created_at = (SELECT created_at FROM books WHERE created_at < 2)`),
    ...
    it('cannot = subquery when multiple columns', () => {
        ...
        assert.throws(
            () =>
                none(
                    `SELECT name FROM books WHERE created_at = (SELECT created_at, name FROM books WHERE created_at < 2)`,
                ),
    ...
    describe.skip('With subqueries accessing parent scope', () => {
        it('fails if multiple columns in predicate', () => {
            ...
            assert.throws(
                () =>
                    many(
                        `SELECT name FROM my_table as t1 WHERE id = (SELECT name, id FROM my_table as t2 WHERE t2.parent_id = t1.id);`,
                    ),
                /subquery must return only one column/,
            );
        });
        ...
        it('fails if multiple columns in selection', () => {
            ...
            assert.throws(
                () =>
                    many(
                        `SELECT name, (SELECT name FROM my_table as t2 WHERE t2.parent_id = t1.id) FROM my_table as t1`,
                    ),
                /subquery must return only one column/,
            );
        });
