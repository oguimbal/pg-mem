...
describe('Selections', () => {
    ...
    it('can select nothing', () => {
        ...
        expect(many(`select;`)).to.deep.equal([{}]);
    });

    it('can use transformations', () => {
        stuff();
        expect(many(`select * from (select val as xx from test where txt = 'A') x where x.xx >= 1`)).to.deep.equal([
            { xx: 999 },
            { xx: 1 },
        ]);
    });
    ...
    it('executes the right plan on transformations', () => {
        stuff();
        const plan = (db as _IDb).public.explainSelect(
            `select * from (select val as valAlias from test where txt = 'A') x where x.valAlias >= 1`,
        );
    ...
    it('selects case whithout condition', () => {
        simpleDb();
        expect(
            many(`insert into data(id, str) values ('id1', 'SOME STRING'), ('id2', 'other string'), ('id3', 'Some String');
    ...
    it('selects case with disparate types results', () => {
        simpleDb();
        expect(many(`select case when 2 > 1 then 1.5 when 2 < 1 then 1 end as x`)).to.deep.equal([{ x: 1.5 }]);
    });
    ...
    it('supports concat operator', () => {
        expect(many(`select 'a' || 'b' as x`)).to.deep.equal([{ x: 'ab' }]);
    });
    ...
    it('detects ambiguous column selections on aliases', () => {
        ...
        assert.throws(
            () =>
                none(`create table data(id text primary key, str text);
    ...
    it('can select from values', () => {
        expect(many(`select * from (values (1, 'one'), (2, 'two')) as  tbl (num, str);`)).to.deep.equal([
            { num: 1, str: 'one' },
            { num: 2, str: 'two' },
        ]);
    });
    ...
    it('can select qualified from values', () => {
        expect(many(`select tbl.num, tbl.str from (values (1, 'one'), (2, 'two')) as  tbl (num, str);`)).to.deep.equal([
            { num: 1, str: 'one' },
            { num: 2, str: 'two' },
        ]);
    });
    ...
    it('can filter from values', () => {
        expect(
            many(`select * from (values (1, 'one'), (2, 'two')) as  tbl (num, str)  where tbl.num>1;`),
        ).to.deep.equal([{ num: 2, str: 'two' }]);
    });

    it('can select from function with selection', () => {
        expect(many(`select a,b from concat('a') as a join concat('a') as b on b=a`)).to.deep.equal([
            { a: 'a', b: 'a' },
        ]);
    });

    it('can select from function without alias', () => {
        expect(many(`select * from concat('a') as a join concat('a') as b on b=a`)).to.deep.equal([{ a: 'a', b: 'a' }]);
    });

    it('can select from function with alias', () => {
        expect(many(`select * from concat('a') as a join concat('a') as b on a.a=b.b`)).to.deep.equal([
            { a: 'a', b: 'a' },
        ]);
    });
    ...
    it('prefers column seleciton over record selection', () => {
        expect(
            many(`create table test (test text, b text);
                    insert into test values ('a', 'b');
                    select test from test;`),
    ...
    it('does not leak null symbols (bugfix)', () => {
        ...
        select * from test;
        `),
        ).to.deep.equal([{ data: { value: null } }, { data: { value: [null] } }, { data: null }]);
    });

    it('can select raw values', () => {
        expect(many(`values (1), (2)`)).to.deep.equal([{ column1: 1 }, { column1: 2 }]);
        expect(many(`values (1, 2)`)).to.deep.equal([{ column1: 1, column2: 2 }]);
    });
    ...
    it('can map column names 1', () => {
        ...
        `),
        ).to.deep.equal([{ a: 'id', b: 'name', value: 'value' }]);
    });
    ...
    it('map column names with conflict', () => {
        ...
        `),
        ).to.deep.equal([{ a: 'id', value: 'name', value1: 'value' }]);
    });
    ...
    it('bugfix on AND & OR operators', () => {
        ...
        expect(
            many(`CREATE TABLE test(id serial, a int, b int);
