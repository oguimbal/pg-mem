...
describe('Custom functions', () => {
    ...
    it('can use sql language as table with * subselection', () => {
        expect(
            many(`CREATE FUNCTION test_fn() RETURNS  table (val text) stable
    ...
    it('can use sql language as table with column subselection', () => {
        expect(
            many(`CREATE FUNCTION test_fn() RETURNS  table (a text, b text) stable
    ...
    it('can reuse table function with different arguments in same statement', () => {
        expect(
            many(`CREATE FUNCTION test_fn(arg text) RETURNS  table (sentence text) stable
    ...
    it('can reuse value function with different arguments in same statement', () => {
        expect(
            many(`CREATE FUNCTION test_fn(arg text) RETURNS  text
                AS $$ select 'hello ' || arg $$
                LANGUAGE SQL;
    ...
    it('can use arguments in table function', () => {
        expect(
            many(`CREATE FUNCTION test_fn(arg text) RETURNS  table (val text) stable
    ...
    describe('should check things', () => {
        it('cannot change type of function', () => {
            ...
            assert.throws(
                () =>
                    none(`CREATE OR REPLACE FUNCTION test_fn() RETURNS INT
                AS $$ select 42 $$
                LANGUAGE SQL;`),
        ...
        it('cannot change args of function', () => {
            ...
            assert.throws(
                () =>
                    none(`CREATE OR REPLACE FUNCTION test_fn(new_arg text) RETURNS TEXT
                AS $$ select 'hello' $$
                LANGUAGE SQL;`),
                /cannot change name of input parameter "arg"/,
            );
        });
        ...
        it('checks column count on single values', () => {
            assert.throws(
                () =>
                    none(`CREATE FUNCTION test_fn() RETURNS INT
        ...
        it('checks column count on table values', () => {
            assert.throws(
                () =>
                    none(`CREATE FUNCTION test_fn() RETURNS table (a int, b int) stable
            AS $$ select * from (values('a') ) as foo(val) $$
            LANGUAGE SQL`),
