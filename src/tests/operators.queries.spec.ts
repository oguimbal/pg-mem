...
describe('Operators', () => {
    ...
    it('timestamp + interval', () => {
        ...
        const dt = result[0]?.dt;
        assert.instanceOf(dt, Date);
        expect(moment(dt).startOf('second').toISOString()).to.equal(
            moment.utc().startOf('second').add(1, 'day').toISOString(),
        );
    });
    ...
    describe('@> operator', () => {
        ...
        it('on array inverted', () => {
            expect(
                many(`create table test(id text, vals jsonb);
    ...
    describe('LIKE operators', () => {
        it('executes like', () => {
            ...
            ).to.deep.equal([{ val: 'foo' }, { val: 'foobar' }]);
        });
        ...
        it('executes ilike', () => {
            ...
            ).to.deep.equal([{ val: 'foo' }, { val: 'foobar' }, { val: 'FOOBAR' }]);
        });
        ...
        it('executes not like', () => {
            ...
            ).to.deep.equal([{ val: 'bar' }, { val: 'FOOBAR' }]);
        });
        ...
        it('executes not ilike', () => {
            ...
            ).to.deep.equal([{ val: 'bar' }]);
        });
    });

    it('executes array index', () => {
        expect(
            many(`create table test(val integer[]);
    ...
    describe('Between operator', () => {
        ...
        it('uses index while using between', () => {
            ...
            expect(got).to.deep.equal([{ num: 1 }, { num: 50 }, { num: 100 }]);
        ...
        it('uses index while using not between', () => {
            preventSeqScan(db);
            expect(
                many(`create table test(num integer primary key);
                            insert into test values (0), (1), (50), (100), (101);
                            select * from test where num not between 1 and 100;`),
            ).to.deep.equal([{ num: 0 }, { num: 101 }]);
