...
describe('Group-by', () => {
    ...
    it('supports ordering with aggregated queries', () => {
        expect(
            many(`create table example(a int, b int);
    ...
    describe('can group by alias', () => {
        ...
        it('case 1', () =>
            expect(many(`SELECT field FROM test GROUP BY field ORDER BY FIELD`).map((x) => x.field)).to.deep.equal([
                1, 2, 3,
            ]));
        ...
        it('case 2', () =>
            expect(
                many(`SELECT field aliased FROM test GROUP BY aliased order by field`).map((x) => x.aliased),
            ).to.deep.equal([1, 2, 3]));

        it('case 3', () =>
            expect(
                many(`SELECT field aliased FROM test GROUP BY aliased order by aliased`).map((x) => x.aliased),
            ).to.deep.equal([1, 2, 3]));
        ...
        it('case 5', () =>
            expect(
                many(`SELECT -field aliased FROM test GROUP BY -field order by -field`).map((x) => x.aliased),
            ).to.deep.equal([-3, -2, -1]));
    });
    ...
    it('prefers non aliased group when ambiguous', () => {
        ...
        expect(many(`SELECT field aliased, (field > 2) field FROM test GROUP BY field order by aliased`)).to.deep.equal(
            [
    ...
    it('allows group by on expression', () => {
        ...
        expect(many(`SELECT (field > 2) field FROM test GROUP BY field >2 order by field`)).to.deep.equal([
            { field: false },
            { field: true },
        ]);
    });
    ...
    it.skip('cannot order by on non grouped field', () => {
        ...
        assert.throws(
            () => many(`SELECT -field FROM test GROUP BY  -field ORDER BY field`),
    ...
    it.skip('fails on column not in group by computation', () => {
        ...
        assert.throws(
            () => many(`SELECT field FROM test GROUP BY  -field`),
            / must appear in the GROUP BY clause or be used in an aggregate function/,
        );
    });
    ...
    it('cannot group on aliased computation', () => {
        ...
        assert.throws(
            () => many(`SELECT field aliased FROM test GROUP BY  -aliased`),
