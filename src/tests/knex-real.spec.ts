...
describe('Knex', () => {
    ...
    it('bugfix on joins', async () => {
        ...
        const result = await knex('TableGroups').innerJoin('TableUsersGroup', 'group_id', 'id').where('user_id', 'uid');

        expect(result).to.deep.equal([{ id: 'gid', name: 'gname', user_id: 'uid', group_id: 'gid' }]);
    });
