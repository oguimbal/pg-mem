...
describe('Simple queries', () => {
    ...
    it('handles show', () => {
        expect(many(`show server_version`)).to.deep.equal([{ server_version: '12.2 (pg-mem)' }]);
    });
    ...
    it('can select twice the same colmun', () => {
        simpleDb();
        expect(
            many(`insert into data(id) values ('key');
    ...
    it('can select NOT', () => {
        expect(many(`select not true as v`)).to.deep.equal([{ v: false }]);
    });

    it('can select negate', () => {
        expect(many(`select -(42) as v`)).to.deep.equal([{ v: -42 }]);
    });
    ...
    it('OR bugfix when there is a null column', () => {
        expect(
            many(`CREATE TABLE IF NOT EXISTS test(
    ...
    it('insert returning', () => {
        expect(
            many(`create table test(id serial primary key, val text, nl text);
    ...
    it('supports bigserial data type', () => {
        expect(
            many(`create table test(id bigserial primary key, val text, nl text);
                                insert into test(val) values ('a'), ('b') returning id, val;`),
        ).to.deep.equal([
            { id: 1, val: 'a' },
            { id: 2, val: 'b' },
    ...
    it('can create two tables with different casing', () => {
        ...
        expect(many('select * from example')).to.deep.equal([{ id: 'one' }, { id: 'two' }]);
        expect(many('select * from Example')).to.deep.equal([{ id: 'one' }, { id: 'two' }]);
        expect(many('select * from "Example"')).to.deep.equal([{ val: 'three' }]);
    });
    ...
    it('can select now()', () => {
        expect(many(`select now()`)[0]).to.have.property('now');
    });
    ...
    it('bugfix on info schema', () => {
        ...
        expect(
            many(
                `SELECT count(*) as hasUsersTable FROM information_schema.tables WHERE table_type = 'BASE TABLE' AND table_catalog = 'test' AND table_name = 'users';`,
            ),
    ...
    it('can select info columns', () => {
        simpleDb();
        expect(many(`select column_name from information_schema.columns where table_name='data'`)).to.deep.equal([
            { column_name: 'id' },
            { column_name: 'str' },
    ...
    it('supports to_date function', () => {
        expect(many(`select to_date('20170103','YYYYMMDD') as x`)).to.deep.equal([{ x: new Date('2017-01-03') }]);
        expect(many(`select to_date('20170103',null) as x`)).to.deep.equal([{ x: null }]);
        expect(many(`select to_date(NULL, 'YYYYMMDD') as x`)).to.deep.equal([{ x: null }]);
    ...
    it('executes member get text ->>', () => {
        ...
        expect(many(`select val->>'prop' as x from test`)).to.deep.equal([
            { x: 'str' },
            { x: '42' },
            { x: `[42,"val"]` },
        ]);
    ...
    it('executes member get text ->', () => {
        ...
        expect(many(`select val->'prop' as x from test`)).to.deep.equal([{ x: 'str' }, { x: 42 }, { x: [42] }]);
    });

    it('does not cast null to int', () => {
        assert.throws(
            () => none(`select (('{"val":null}'::jsonb) -> 'val')::int`),
            /cannot cast jsonb null to type integer/,
        );
    });

    it('does casts null to int when using ->>', () => {
        expect(many(`select (('{"val":null}'::jsonb) ->> 'val')::int val`)).to.deep.equal([{ val: null }]);
    });

    it('auto increments values', () => {
        expect(
            many(`create table test(id serial, txt text);
    ...
    it('not null does not accept null values', () => {
        assert.throws(() =>
            none(`create table test(txt text not null);
    ...
    it('can create columns not null with default', () => {
        expect(
            many(`create table test(id text, val text not null default 'def');
    ...
    it('can create columns moving constant defaults', async () => {
        const orig = many(`create table test(id text, time timestamp default now());
                    insert into test(id) values ('id1') returning time;`).map((x) => x.time)[0];
        assert.instanceOf(orig, Date);
        await new Promise((done) => setTimeout(done, 5)); // wait 5 ms
        const newtime = many(`insert into test(id) values ('id2') returning time;`).map((x) => x.time)[0];
    ...
    describe('ANY() and operators', () => {
        for (const x of [
            ...
            it('can execute ANY(): ' + x.query, () => {
                expect(many(x.query)).to.deep.equal([x.result]);
            });
        }

        it('can execute any on a selection', () => {
            expect(
                many(`create table vals(val int);
                            insert into vals values (0), (1), (50), (100), (101);
                            select 50 = any(select * from vals) as x`),
            ).to.deep.eq([{ x: true }]);
            expect(many(`select 42 = any(select * from vals) as x`)).to.deep.eq([{ x: false }]);
        });
    });
    ...
    describe('substring query as range', () => {
        for (const [k, v] of [
            ...
            it(k, () => {
                expect(many(k)).to.deep.equal([{ v }]);
            });
        }
        it('fails with negative for', () => {
            assert.throws(
                () => many(`select substring('012345678' for -1) as v`),
    ...
    it('supports substring() as classic function range', () => {
        expect(many(`select substring('012345678', 2, 3) as v`)).to.deep.equal([{ v: '123' }]);
        expect(many(`select substring('012345678', 5) as v`)).to.deep.equal([{ v: '45678' }]);
    });
    ...
    describe('overlay query as range', () => {
        ...
        it('fails with negative from', () => {
            assert.throws(
                () => many(`select overlay('12345678' placing 'ab' from -2) as v`),
                /negative substring length not allowed/,
            );
            assert.throws(
                () => many(`select overlay('12345678' placing 'ab' from 0) as v`),
                /negative substring length not allowed/,
            );
        });
    });
    ...
    it('can select list', () => {
        expect(many(`select (1, 2) as v`)).to.deep.equal([{ v: [1, 2] }]);
    });

    it('can convert list to text', () => {
        expect(many(`select (1, 2)::text as v`)).to.deep.equal([{ v: '(1,2)' }]);
    });

    it('supports select ARRAY[]', () => {
        expect(many(`select ARRAY[1, 2] as v`)).to.deep.equal([{ v: [1, 2] }]);
    });

    it('can convert array to text', () => {
        expect(many(`select ARRAY[1, 2]::text as v`)).to.deep.equal([{ v: '{1,2}' }]);
    });

    it('can execute ANY on ARRAY[]', () => {
        expect(
            many(`create table tbl (id text);
    ...
    it('should not be able to determine the type of an empty array', () => {
        ...
        assert.throws(
            () => none(`INSERT INTO example (members) VALUES (ARRAY[])`),
