...
describe('Sequences', () => {
    ...
    it('fails to get currval without initialization', () => {
        ...
        assert.throws(
            () => none(`select currval('test');`),
            /currval of sequence "test" is not yet defined in this session/,
        );
    });

    it('can define custom sequences', () => {
        none(
            `CREATE SEQUENCE if not exists public.test START WITH 40 INCREMENT BY 2 NO MINVALUE NO MAXVALUE CACHE 1 as bigint cycle`,
        );
    ...
    it('cannot reach its maximum', () => {
        expect(
            many(`create sequence test maxvalue 3;
                select nextval('test');
                select nextval('test');
                select nextval('test');`),
    ...
    it('can restart sequence with 0', () => {
        ...
        expect(one(`select nextval('test');`)).to.deep.equal({ nextval: 0 });
        expect(one(`select currval('test');`)).to.deep.equal({ currval: 0 });
    });
    ...
    it('cannot restart sequence with too low value', () => {
        ...
        // restart sequence
        assert.throws(
            () => none(`ALTER SEQUENCE test RESTART WITH 4;`),
            /RESTART value \(4\) cannot be less than MINVALUE \(5\)/,
        );

        none(`ALTER SEQUENCE test RESTART WITH 5;`);

        expect(one(`select nextval('test');`)).to.deep.equal({ nextval: 5 });
    });

    it('can restart sequence after usage', () => {
        expect(
            one(`create sequence test;
                    select setval('test', 41);`),
        ).to.deep.equal({ setval: 41 });

        // checks on currval & nextval
        expect(one(`select currval('test');`)).to.deep.equal({ currval: 41 });
        expect(one(`select nextval('test');`)).to.deep.equal({ nextval: 42 });
        expect(one(`select currval('test');`)).to.deep.equal({ currval: 42 });

        // restart sequence
        none(`ALTER SEQUENCE test RESTART WITH 1;`);

        // checks on currval & nextval
        expect(one(`select currval('test');`)).to.deep.equal(
            { currval: 42 },
            'Current value should not have moved after reset',
        );
        expect(one(`select nextval('test');`)).to.deep.equal({ nextval: 1 });
        expect(one(`select currval('test');`)).to.deep.equal({ currval: 1 });
    });
    ...
    it('can restart sequence when not used', () => {
        ...
        expect(one(`select nextval('test');`)).to.deep.equal({ nextval: 5 });
        expect(one(`select currval('test');`)).to.deep.equal({ currval: 5 });
    });
});
