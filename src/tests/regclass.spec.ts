...
describe('regclass', () => {
    ...
    it('can select pg_catalog tables as regclass', () => {
        expect(many(`select 'pg_class'::regclass`)).to.deep.equal([{ regclass: 'pg_class' }]);
    });
    ...
    it('can cast existing from int', () => {
        ...
        assert.isNumber(rt);
        expect(many(`select ${rt}::regclass as asint, '${rt}'::regclass as asstr`)).to.deep.equal([
            {
                asint: rt,
                asstr: 'test',
            },
    ...
    it('can cast non existing from int', () => {
        expect(many(`select 42424242::regclass`)).to.deep.equal([
            {
                regclass: 42424242,
            },
        ]);
