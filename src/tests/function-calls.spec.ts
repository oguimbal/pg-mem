...
describe('Functions', () => {
    ...
    it('does not pay attention to casing', () => {
        expect(many(`SELECT CONCAT('a', 'b', 'c')`)).to.deep.equal([{ concat: 'abc' }]);
        expect(many(`SELECT ConCat('a', 'b', 'c')`)).to.deep.equal([{ concat: 'abc' }]);
        assert.throws(() => many(`SELECT "ConCat"('a', 'b', 'c')`), /does not exist/);
    });

    it('accepts nulls in concat', () => {
        expect(many(`select concat('text-', null, 123, null, '-end');`)).to.deep.equal([{ concat: 'text-123-end' }]);
    });
    ...
    it('can declare & call function', () => {
        ...
        expect(many(`select "sayHello"('world')`)).to.deep.equal([{ sayHello: 'hello world' }]);
    });
    ...
    it('can compile kind-of plv8', () => {
        ...
        expect(
            many(`create or replace function calc_plv8(x int, y int, func text)
    returns int
    as
    $$
    if (func === '+'){
    ...
    it('does not call when has null argument', () => {
        ...
        expect(many(`select myfn(null)`)).to.deep.equal([{ myfn: null }]);
    });
    ...
    it('calls when has null argument and told it to', () => {
        ...
        expect(many(`select myfn(null)`)).to.deep.equal([{ myfn: 'hi !' }]);
    });
    ...
    it('[bugfix] supports coalesce() with implicit cast arguments', () => {
        ...
        assert.throws(
            () => none(`select COALESCE(42, '2021-12-07T13:49:53.458Z'::timestamp)`),
            /COALESCE types integer and timestamp cannot be matched/,
        );
    });
