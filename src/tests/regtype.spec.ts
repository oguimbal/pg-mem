...
describe.skip('regtype', () => {
    ...
    it('can select native type as regtype', () => {
        expect(many(`select 'text'::regtype`)).to.deep.equal([{ regtype: 'text' }]);
    });

    it('can select native with arg type as regtype', () => {
        expect(
            many(
                `select 'varchar(42)'::regtype as a, 'varchar(51)'::regtype as b, 'varchar(42)'::regtype::integer as c, 'varchar(51)'::regtype::integer as d`,
            ),
        ).to.deep.equal([
            {
                a: 'character varying',
    ...
    it('can cast back native to integer', () => {
        expect(many(`select 'text'::regtype::integer`)).to.deep.equal([
            {
                integer: 123, // todo
            },
        ]);
    ...
    it('can cast existing from int', () => {
        ...
        assert.isNumber(rt);
        expect(many(`select ${rt}::regtype as `)).to.deep.equal([
            {
                regtype: 'test',
            },
        ]);
    ...
    it('can cast non existing from int', () => {
        expect(many(`select 42424242::regtype`)).to.deep.equal([
            {
                regtype: 42424242,
            },
        ]);
