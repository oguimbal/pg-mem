...
describe('Data types', () => {
    ...
    describe('uuid', () => {
        ...
        for (const v of [
            'A0EEBC99B-9C0B-4EF8-BB6D-6BB9BD380A11',
            '{a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11',
            'a0eebc99-9c0b4ef8bb6d6bb9bd380a11}',
    ...
    describe('time', () => {
        it('can be casted', () => {
            expect(many(`select '00:35:19.383683'::time without time zone`)).to.deep.equal([
                {
    ...
    describe('inet', () => {
        it('can be inserted', () => {
            ...
            expect(
                many(`CREATE TABLE example ( ip INET);
    ...
    describe('jsonb', () => {
        it('[bugfix] supports negative array indexation', () => {
            expect(
                many(`select data -> 'val' ->  -1 as value from (values ('{"val": [1,2]}'::jsonb)) v(data)`),
        ...
        it('[bugfix] cannot cast implicitely jsonb boolean to pg boolean', () => {
            ...
            // this was throwing
            assert.throws(
                () => many(`select * from test where data -> 'val'`),
                /argument of WHERE must be type boolean, not type jsonb/,
            );
            assert.throws(
                () => many(`select * from test where data -> 'val' OR 1=1`),
                /argument of OR must be type boolean, not type jsonb/,
            );
        });
    });
});
