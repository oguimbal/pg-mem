...
describe('Inserts', () => {
    ...
    it('handles on on conflict do nothing', () => {
        ...
        expect(many('select * from test')).to.deep.equal([{ id: 'x' }]);
    });
    ...
    it('handles on on conflict do update single set', () => {
        ...
        // just check that reinserting does not work
        assert.throws(
            () => none(`insert into test values ('x');`),
        ...
        expect(many('select * from test')).to.deep.equal([{ id: 'x', val: 'new' }]);
    });
    ...
    it('handles referencing other values in update', () => {
        expect(
            many(`create table test(id text primary key, a text, b text);
                        insert into test values ('x', 'oldA', 'oldB');
    ...
    it('handles implicit conversions on conflict set', () => {
        expect(
            many(`create table test(id text primary key, val jsonb);
    ...
    it('handles setting with alias on conflict', () => {
        expect(
            many(`create table test(id text primary key, a text unique, b text);
                        insert into test values ('x', 'oldA', 'oldB');
    ...
    it('handles on conflict with muliple columns indices', () => {
        expect(
            many(`create table test(ka text, kb integer, val text,  primary key (ka, kb));
    ...
    it('ensures serials are transactional', () => {
        expect(
            many(`create table test(id serial primary key, val text);
    ...
    it('[bugfix] allows returning statement', () => {
        expect(
            many(`CREATE TABLE "user" ("id" SERIAL NOT NULL, "name" text NOT NULL, CONSTRAINT "PK_cace4a159ff9f2512dd42373760" PRIMARY KEY ("id"));
    ...
    it('can override value when generated by default', () => {
        expect(
            many(`create table test(id int  GENERATED BY DEFAULT AS IDENTITY, val text);
    ...
    it('sets default when not provided', () => {
        expect(
            many(`create table test(id text, n int default 0);
                insert into test(id) values ('x');
                select * from test;`),
        ).to.deep.equal([{ id: 'x', n: 0 }]);
    });

    it('sets default when explicitely told so', () => {
        expect(
            many(`create table test(n int default 0);
    ...
    it('cannot override value when generated always', () => {
        ...
        assert.throws(
            () => none(`insert into test(id,val) overriding user value values (42,'val');`),
    ...
    it('can force override value when generated by default', () => {
        expect(
            many(`create table test(id int  GENERATED ALWAYS AS IDENTITY, val text);
    ...
    describe('insert into select', () => {
        it('can insert into select', () => {
            expect(
                many(`create table test(a varchar(4), b int, c jsonb);
        ...
        it('cannot insert into select when not implicitely convertible', () => {
            ...
            assert.throws(
                () => none(`insert into test (select * from (values ('a', 42, '[]') ) as t)`),
                /column "c" is of type jsonb but expression is of type text/,
            );
        });
        ...
        it('checks that insert values has enough columns', () => {
            none(`create table test(a varchar(4), b int, c jsonb);`);
            assert.throws(
                () => none(`insert into test(a) (select * from (values ('a', 42, '[]') ) as t)`),
        ...
        it('preserves null jsonb alues', () => {
            expect(
                many(`create table test(val jsonb);
