...
describe('Indices', () => {
    ...
    it('uses indexes for null values', () => {
        ...
        expect(got).to.deep.equal([
            { id: 'id1', str: null, otherstr: null },
            { id: 'id3', str: null, otherstr: null },
        ]);
    });
    ...
    describe('[bugfix #160] indexes with default values', () => {
        ...
        it('insert into select is OK with partial indexes', () => {
            ...
            // was throwing n°2
            none(
                `insert into test_table(id,unique_data,deleted_at) (select id || 'bis', unique_data, deleted_at from test_table)`,
            );
        });
        ...
        it('rejects two default not null values on non partial', () => {
            ...
            assert.throws(
                () => none(`insert into test_table ("id", "unique_data") VALUES('2', default)`),
                /duplicate key value violates unique constraint/,
            );
        });
    ...
    it('can use an index on an aliased selection not aliased var', () => {
        ...
        expect(got).to.deep.equal([{ val: 2 }, { val: 3 }]);
    });
    ...
    it('can use an index on an aliased selection & aliased var', () => {
        ...
        const explain = db.public.explainSelect(
            `select * from (select val as xx from test where txt != 'A') x where x.xx > 1`,
        );
        assert.deepEqual(explain, {
            _: 'seqFilter',
        ...
        expect(got).to.deep.equal([{ xx: 2 }, { xx: 3 }]);
    });
    it('can use an index on an aliased "!=" selection', () => {
        // preventSeqScan(db);
        expect(
            many(`create table test(txt text, val integer);
                create index on test(txt);
                create index on test(val);
                insert into test values ('A', 999);
                insert into test values ('A', 0);
    ...
    it('can use implicit cast index on index', () => {
        expect(
            many(`create table example(id int primary key);
                insert into example(id) values (1);
                select * from example where id='1';`),
        ).to.deep.equal([{ id: 1 }]);
        expect(many(`select * from example where id>'0';`)).to.deep.equal([{ id: 1 }]);
        expect(many(`select * from example where id<'3';`)).to.deep.equal([{ id: 1 }]);
        expect(many(`select * from example where id>'1';`)).to.deep.equal([]);
    });
