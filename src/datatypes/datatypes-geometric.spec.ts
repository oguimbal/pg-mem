...
describe('Datatypes - geometric', () => {
    ...
    it('can select points', () => {
        expect(many(`select '1,2'::point`)).to.deep.equal([{ point: { x: 1, y: 2 } }]);
    });
    ...
    it('should return copies', () => {
        ...
        // check immutability
        expect(many(`select * from test`)).to.deep.equal([{ pt: { x: 1, y: 2 } }]);
    });

    it('can convert points back to text', () => {
        expect(many(`select '1,2'::point::text`)).to.deep.equal([{ text: '(1,2)' }]);
    });

    it('supports path equality, which always return true', () => {
        expect(many(`select '1,2,3,4'::path = '1,2,3,4'::path as eq`)).to.deep.equal([{ eq: true }]);
        expect(many(`select '55,2,3,4'::path = '3,5,55,4'::path as eq`)).to.deep.equal([{ eq: true }]); // YES! "TRUE" ! Try it... that always returns true.
    });

    it('supports path strict inequalitites, which always return false', () => {
        expect(many(`select '1,2,3,4'::path < '1,2,3,4'::path as eq`)).to.deep.equal([{ eq: false }]);
    });
